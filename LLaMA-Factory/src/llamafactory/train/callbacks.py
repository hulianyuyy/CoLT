...
class CopyFilesOnTrainBeginCallback(TrainerCallback):
    r"""A callback for copying user-specified files into output_dir when training starts."""
    ...
    def on_train_begin(self, args: "TrainingArguments", state: "TrainerState", control: "TrainerControl", **kwargs):
        ...
        for source_path in self.source_paths:
            ...
            if not os.path.isfile(resolved_source):
                logger.warning_rank0(f"Skip copying missing file: {resolved_source}.")
                continue

            target_path = os.path.join(args.output_dir, os.path.basename(resolved_source))
            shutil.copy2(resolved_source, target_path)
            logger.info_rank0(f"Copied {resolved_source} to {target_path}.")
